...
class VeSync(Account):
    ...
    def swap_to_eth(self, from_token: str, to_token: str, amount: int, slippage: int):
        ...
        self.approve(amount, token_address, Web3.to_checksum_address(VESYNC_CONTRACTS["router"]))
    ...
    def swap(
        ...
        logger.info(
            f"[{self.account_id}][{self.address}] Swap on VeSync – {from_token} -> {to_token} | {amount} {from_token}"
        )
